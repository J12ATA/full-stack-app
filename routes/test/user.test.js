...
describe("USER", () => {
  ...
  describe("POST /api/users/add_user", () => {
    it("should accept and add a valid new user", done => {
      return request(app).post("/api/users/add_user").send(newDoc).then(res => {
    ...
    it("Should reject adding a user with missing required fields", done => {
      ...
      return request(app).post("/api/users/add_user").send(badDoc).then(res => {
  ...
  describe("GET /api/users/get_all_users", () => {
    it("should return JSON array", done => {
      return request(app).get("/api/users/get_all_users").expect(200).then(res => {
  ...
  describe("GET /api/users/:_id", () => {
    it("should return a user object", done => {
      return request(app).get(`/api/users/${testId}`).expect(200).then(res => {
    ...
    it("should return a user object with requested :_id", done => {
      return request(app).get(`/api/users/${testId}`).expect(200).then(res => {
        const foundDoc = res.body;
    ...
    it("should 400 when requesting a bad :_id", done => {
      return request(app).get(`/api/users/${badId}`).expect(400).then(res => {
        expect(res.error.text).toBe(`Bad Request: No user found with _id ${badId}`);
        done();
      });
    });
  });
  ...
  describe("PUT /api/users/:_id", () => {
    it("allows updates to props other than _id", done => {
      return request(app).put(`/api/users/${testId}`).send(updateDoc).then(res => {
        expect(res.status).toBe(200);
        expect(res.body.message).toBe("Success!");
        done();
      });
    ...
    it("rejects updates to _id prop", done => {
      return request(app).put(`/api/users/${testId}`).send({ _id: "LOL" }).then(res => {
        expect(res.status).toBe(400);
    ...
    it("should 400 when updating a bad :_id", done => {
      return request(app).put(`/api/users/${badId}`).send(updateDoc).then(res => {
        expect(res.status).toBe(400);
        expect(res.error.text.startsWith("Update failed")).toBe(true);
        done();
      });
  ...
  describe("DELETE /api/users/:_id", () => {
    it("deletes when given a valid _id", done => {
      return request(app).delete(`/api/users/${testId}`).then(res => {
        expect(res.status).toBe(200);
        expect(res.body.message).toBe("Success!");
    ...
    it("responds w/ error if given invalid _id", done => {
      return request(app).delete(`/api/users/${badId}`).then(res => {
        expect(res.status).toBe(400);
