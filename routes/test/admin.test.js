...
describe("ADMIN", () => {
  ...
  const { expectedProps, newDoc, excessDoc, updateDoc } = collection;
  let testId, badId;
  ...
  describe("POST /api/admin/add_admin", () => {
    it("Should reject adding an admin with missing required fields", done => {
      ...
      return request(app).post("/api/admin/add_admin").send(badDoc).then(res => {
    ...
    it("should accept and add a valid new admin", done => {
      return request(app).post("/api/admin/add_admin").send(newDoc).then(res => {
    ...
    it("Should reject adding a second admin", done => {
      return request(app).post("/api/admin/add_admin").send(excessDoc).then(res => {
        expect(res.status).toBe(400);
  ...
  describe("GET /api/admin/get_all_admins", () => {
    it("should return JSON array", done => {
      return request(app).get("/api/admin/get_all_admins").expect(200).then(res => {
  ...
  describe("GET /api/admin/:_id", () => {
    it("should return an admin object", done => {
      return request(app).get(`/api/admin/${testId}`).expect(200).then(res => {
    ...
    it("should return an admin object with requested :_id", done => {
      return request(app).get(`/api/admin/${testId}`).expect(200).then(res => {
        const foundDoc = res.body;
    ...
    it("should 400 when requesting a bad :_id", done => {
      return request(app).get(`/api/admin/${badId}`).expect(400).then(res => {
        expect(res.error.text).toBe(`Bad Request: No admin found with _id ${badId}`);
        done();
      });
    });
  });
  ...
  describe("PUT /api/admin/:_id", () => {
    it("allows updates to props other than _id", done => {
      return request(app).put(`/api/admin/${testId}`).send(updateDoc).then(res => {
        expect(res.status).toBe(200);
        expect(res.body.message).toBe("Success!");
        done();
      });
    ...
    it("rejects updates to _id prop", done => {
      return request(app).put(`/api/admin/${testId}`).send({ _id: "LOL" }).then(res => {
        expect(res.status).toBe(400);
    ...
    it("should 400 when updating a bad :_id", done => {
      return request(app).put(`/api/admin/${badId}`).send(updateDoc).then(res => {
        expect(res.status).toBe(400);
        expect(res.error.text.startsWith("Update failed")).toBe(true);
        done();
      });
  ...
  describe("DELETE /api/admin/:_id", () => {
    it("deletes when given a valid _id", done => {
      return request(app).delete(`/api/admin/${testId}`).then(res => {
        expect(res.status).toBe(200);
        expect(res.body.message).toBe("Success!");
    ...
    it("responds w/ error if given invalid _id", done => {
      return request(app).delete(`/api/admin/${badId}`).then(res => {
        expect(res.status).toBe(400);
